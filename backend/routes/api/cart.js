...
router.get('/user/:userId', requireUser, async (req, res) => {
    try {
        ...
        const cart = await Cart.findOne({ user: authenticatedUserId }).populate('items.shoeId', '_id name price photoUrl');
        res.json(cart);
    } catch (err) {
...
router.post('/', requireUser, async (req, res) => {
    try {
        ...
        let cart = await Cart.findOne({ user: authenticatedUserId }).populate('items.shoeId', '_id name price photoUrl');

        if (!cart) {
            cart = new Cart({ user: authenticatedUserId, items: [] });
        } else {
            cart = await cart.populate('items.shoeId', '_id name price photoUrl');
        }
